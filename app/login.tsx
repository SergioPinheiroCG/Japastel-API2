    ...
    const handleEntrar = async () => {
        if (validateForm()) {
            try {
                ...
                if (response.ok) {
                    // Login bem-sucedido, armazene o token ou dados do usuário
                    await AsyncStorage.setItem('userToken', data.token); // Exemplo de como guardar um token
                    router.push('/(tabs)/home'); // Redireciona para a tela inicial
                } else {
                    // Caso a resposta do servidor seja um erro
